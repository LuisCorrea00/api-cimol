...
get = async () => {
    ...
    users = await mysql.query(
        'SELECT p.nome, p.email, u.senha FROM usuario u JOIN pessoa p ON p.id_pessoa=u.pessoa_id_pessoa'
    );
    return users;
};
            ...
            if (usuarios[0].id) {
                ...
                if (usuarios[0].coordenador > 0) {
                    sql =
                        ...
                        'WHERE cc.professor_pessoa_id_pessoa=' +
                        usuarios[0].id;
                    console.log(sql);
...
verifyJWT = async (token, perfil) => {
    ...
    if (resp.auth) {
        sql =
            'SELECT perfil FROM usuario WHERE pessoa_id_pessoa=' + resp.idUser;
