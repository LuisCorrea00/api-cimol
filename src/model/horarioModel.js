...
getHorarios = async()=>{
    return await mysql.query("SELECT DISTINCT DD.dia AS Dia, DD.dia_turno AS Turno, H.hora AS Horario, D.nome AS Disciplina, D.id_disciplina AS id_disciplina, AMB.tipo AS Ambiente, P.nome AS Professor, T.nome_turma AS Turma, T.id_turma AS id_turma, T.tamanho AS Tamanho"
        ...
        + " WHERE DD.dia = dia AND DD.dia_turno = dia_turno"
        + " ORDER BY H.hora, DD.dia_turno, D.nome;"
    );
}

module.exports={getByTurma, getByProfessor, getHorarios}